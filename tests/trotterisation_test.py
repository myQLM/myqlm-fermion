...
class TestMakeTrotterSliceSimple(unittest.TestCase):
    def check_trotterisation(self, hpq, hpqrs, tol):
        ...
        self.assertAlmostEqual(np.linalg.norm(U_exact - U_trotter),
                               0,
                               delta=tol)

    def test_make_trotter_slice_jw_2_qbits(self):
        nqbits = 2
        hpq = np.zeros((nqbits, nqbits))
        hpqrs = np.zeros((nqbits, nqbits, nqbits, nqbits))
        hpq[0][0] = 1
        self.check_trotterisation(hpq, hpqrs, tol=1e-10)

    def test_make_trotter_slice_jw_2_qbits_2(self):
        nqbits = 2
        hpq = np.zeros((nqbits, nqbits))
        hpqrs = np.zeros((nqbits, nqbits, nqbits, nqbits))
        hpq[0][0] = 1
    ...
    def test_make_trotter_slice_jw_2_qbits_3(self):
        nqbits = 2
        hpq = np.zeros((nqbits, nqbits))
        hpqrs = np.zeros((nqbits, nqbits, nqbits, nqbits))
        hpq[0][1] = 1
    ...
    def test_make_trotter_slice_jw_4_qbits(self):
        nqbits = 4
        hpq = np.zeros((nqbits, nqbits))
        hpqrs = np.zeros((nqbits, nqbits, nqbits, nqbits))
...
class TestMakeTrotterSlice(unittest.TestCase):

    def checks_trotter_slice(self, hpq, hpqrs):
        ...
        self.assertTrue(np.linalg.norm(a - b) < TOL1)

        return a

    def test_hpq_number_operator(self):
        number_qubits = 2
        hpq = np.zeros((2, 2))
        ...
        a = self.checks_trotter_slice(hpq, hpqrs)
        self.assertTrue(np.linalg.norm(c - a) < TOL1)

    def test_hpq_excitation_operator(self):
        number_qubits = 2
    ...
    def test_hpqqp_coulomb_operator(self):
        number_qubits = 3
        hpq = np.zeros((number_qubits, number_qubits))
        hpqrs = np.zeros((number_qubits, number_qubits, number_qubits, number_qubits))
        hpqrs[0][2][2][0] = -1.5
    ...
    def test_hpqqr_numberexcitation_operator(self):
        # cas r<q<p
        number_qubits = 3
        hpq = np.zeros((number_qubits, number_qubits))
        hpqrs = np.zeros((number_qubits, number_qubits, number_qubits, number_qubits))
    ...
    def test_hpqrs_doubleexcitation_operator(self):
...
class TestMakeSpinHamiltonianTrotterSlice(unittest.TestCase):

    def check_evolution(self, ising_hamiltonian):
        ...
        self.assertAlmostEqual(np.linalg.norm(a - b), 0, delta=1e-3)

    def test_ZZ_term(self):
        theta = 0.543
        spin_hamiltonian = SpinHamiltonian(2, pauli_terms=[Term(theta, "ZZ", [0, 1])])
        self.check_evolution(spin_hamiltonian)

    def test_XX_term(self):
        theta = 0.3
        spin_hamiltonian = SpinHamiltonian(2, pauli_terms=[Term(theta, "XX", [0, 1])])
        self.check_evolution(spin_hamiltonian)
    ...
    def test_n_terms(self):
        ...
        U_mats = [sp.linalg.expm(-1j * coeff * np.kron(pauli_dict[key[0]],
                                                       pauli_dict[key[1]])) for coeff, key in zip(coeffs, terms)]
        U_mat = np.linalg.multi_dot(U_mats[::-1])
        pauli_terms = [Term(coeff, term, [0, 1]) for coeff, term in zip(coeffs, terms)]
        qrout2 = make_spin_hamiltonian_trotter_slice(Observable(2, pauli_terms=pauli_terms))
        U_mat_circ = get_unitary_from_circuit(qrout2, nqbits)
        # print(terms)
        self.assertAlmostEqual(np.linalg.norm(U_mat - U_mat_circ), 0, delta=1e-13)
