...
class Test_embedding_hamiltonian(unittest.TestCase):
        ...
        def test_risb_embedding(self):
            ...
            int_kernel = 1.0*np.fromfunction(lambda i, j,k,l: (i==j)*(i%2==0)*(k==l)*(k==i+1)*1, (2*Nc, 2*Nc, 2*Nc, 2*Nc), dtype = float)
            int_kernel_sym = 1.0*np.fromfunction(lambda i, j,k,l: (i==j)*(i%2==1)*(k==l)*(k==i-1)*1, (2*Nc, 2*Nc, 2*Nc, 2*Nc), dtype = float)
            ...
            non_int_term = sum([lambda_c[i,j]*dot(d[M+j],d_dag[M+i])
                                     for i,j in itertools.product(list(range(M)),list(range(M)))])\
                                 + sum([D[i,j]*dot(d_dag[i],d[M+j])+conj(D[i,j])*dot(d_dag[M+j],d[i])
                                      for i,j in itertools.product(list(range(M)),list(range(M)))])\
            ...
            self.assertAlmostEqual(min(eig), min(Eig), 7)
