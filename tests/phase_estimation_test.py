...
class TestPEA(unittest.TestCase):
    ...
    def check_all_states(self, H_el_structure, nqbits_phase, n_trotter_steps, n_adiab_steps,
                         E_target, E_range, delta, second_E_range=0.1, user_second_try=False):
        """
        ...
        for init_vec in product(range(2), repeat=n_qubits):
            ...
            qpe_energy, probs = perform_phase_estimation(H_el_structure, nqbits_phase,
                                                         n_trotter_steps, init_vec=init_vec,
                                                         n_adiab_steps=n_adiab_steps,
                                                         E_target=E_target, size_interval=E_range)
            ...
            if not energy_is_found and user_second_try:
                qpe_energy, probs = perform_phase_estimation(H_el_structure, nqbits_phase,
                                                             n_trotter_steps, init_vec=init_vec,
                                                             n_adiab_steps=n_adiab_steps, E_target=qpe_energy,
    ...
    def test_adiabatic_state_prep(self):
        """
        ...
        """
        H_qbasis = transform_to_jw_basis(TestPEA.H_el_structure)
        n_qubits_H = H_qbasis.nbqbits
        n_phase_bits = 2 # set it to 2 just for the QPEs to execute

        # "_hopping" is for the part of the H which has only hpp terms (no hpq or hpqrs)
        H_el_hopping_hpq = np.diag(np.diag(TestPEA.H_el_structure.hpq)) # extract the diagonal and return an empty array but with this diagonal.
        H_el_hopping_f = ElectronicStructureHamiltonian(H_el_hopping_hpq, hpqrs=None, constant_coeff=0.0)  # "_f" stands for fermionic basis
    ...
    def test_hubbard_molecule__from_notebook(self):
        """
        ...
        for state_index in range(len(eigvals)): # one eigenstate per eigenenergy
            ...
            qrout.apply(state_prep(np.array(eigen_state) / state_normalizing_factor), [0, 1, 2, 3])

            qpe_energy, probs = perform_phase_estimation(hamilt, nqbits_phase, n_trotter_steps,
                                                         n_adiab_steps=n_adiab_steps,
                                                         E_target=E_target,
                                                         size_interval=size_interval,
                                                         init_vec=qrout)
#             print(eigvals[state], qpe_energy, abs(eigvals[state] - qpe_energy) <= delta)
    ...
    def test_rough_estimate(self):
        ...
        assert self.check_all_states(TestPEA.H_el_structure, nqbits_phase,
                                     n_trotter_steps, n_adiab_steps,
                                     E_target, E_range, delta) <= 1

    def test_precise_estimate(self):
        """
        Test if aiming for more precise energies works if one uses the already found energy as E_target.
        """
        # Define the QPE parameters
        nqbits_phase = 9 # 9
        n_trotter_steps = 11 # 11
        ...
        assert self.check_all_states(TestPEA.H_el_structure, nqbits_phase,
                                     n_trotter_steps, n_adiab_steps,
                                     E_target, E_range, delta, user_second_try=True) <= 1

    def test_bad_E_target_E_range(self):
        """
        ...
        assert self.check_all_states(TestPEA.H_el_structure_modified, nqbits_phase,
                                     n_trotter_steps, n_adiab_steps,
                                     E_target, E_range, delta) >= 10

    def test_E_target_E_range_adiab_state_prep(self):
        """
        ...
        assert self.check_all_states(TestPEA.H_el_structure_modified, nqbits_phase,
                                     n_trotter_steps, n_adiab_steps,
                                     E_target, E_range, delta, second_E_range, user_second_try=True) <= 2

        # In fact, the test below is misleading as the test above works better with n_adiab_steps = 0..
        # The moment n_adiab_steps is no more 0 - some energies are way too off.
        # So atm we don't show how the state-preparation is beneficial or even useful..
        # A good test though would be to check if this adiabatic stage really produces an eigenstate of
        # the final H - just before the main QPE routine.

#     def test_no_adiab_state_prep(self):
#         """
#         Show that with no adiabatic state preparation, worse energies are found.
#         """
#         # Define the QPE parameters
#         nqbits_phase = 4 # 4
#         n_trotter_steps = 6 # 6
...
#         n_adiab_steps = 0 # 0
#         E_target = TestPEA.constant_E # -14.8
#         E_range = 40 # 40
#         second_E_range = 3

#         assert not self.check_all_states(TestPEA.H_el_structure_modified, nqbits_phase,
#                                          n_trotter_steps, n_adiab_steps,
#                                          E_target, E_range, delta, second_E_range, user_second_try=True) <= 1

    def test_no_init_vec(self):
        """
        ...
        # Perform the QPE
        qpe_energy, probs = perform_phase_estimation(TestPEA.H_el_structure_modified,
                                                     nqbits_phase, n_trotter_steps,
                                                     init_vec=None, n_adiab_steps=n_adiab_steps,
                                                     E_target=E_target, size_interval=E_range)

        # Calculate the corresponding eigenvalues and eigenvectors
        H_spin_mat = TestPEA.H_el_structure_modified.get_matrix()
        eigvals, eigvecs_transposed = np.linalg.eigh(H_spin_mat)
    ...
    def test_init_vec_as_routine(self):
        """
        Show that one can supply a routine preparing a desired initial state and it works.
        """
        # Define the QPE parameters
        ...
        n_qubits_H = TestPEA.H_el_structure_modified.nbqbits
        ...
        qpe_energy, probs = perform_phase_estimation(TestPEA.H_el_structure_modified,
                                                     ...
                                                     n_adiab_steps=n_adiab_steps,
                                                     E_target=E_target, size_interval=E_range)
        assert abs(TestPEA.lowest_E - qpe_energy) <= delta

    def check_basic(self, t_hopping):
        """
        ...
        H_el_structure_2 = ElectronicStructureHamiltonian(H_hubbard.hpq * 2.18,
                                                          H_hubbard.hpqrs * 3.27,
                                                          -4.2)
        # Define the QPE parameters
        nqbits_phase = 4 # 4
        n_trotter_steps = 7 # 7
        ...
        assert self.check_all_states(H_el_structure_2, nqbits_phase, n_trotter_steps, n_adiab_steps,
                                     E_target, E_range, delta) <= 2

    def test_basic_zero_hopping(self):
        self.check_basic(t_hopping=0)

    def test_basic_finite_hopping(self):
        self.check_basic(t_hopping=0.05)

    def test_basic_finite_hopping_2(self):
        self.check_basic(t_hopping=0.1)

    def test_basic_finite_hopping_3(self):
        self.check_basic(t_hopping=0.25)

    def test_basic_finite_hopping_4(self):
        self.check_basic(t_hopping=0.5)

    def test_basic_finite_hopping_5(self):
        self.check_basic(t_hopping=1.0)

    def test_basic_finite_hopping_6(self):
        self.check_basic(t_hopping=1.3)

    def test_basic_finite_hopping_7(self):
        self.check_basic(t_hopping=1.6)
