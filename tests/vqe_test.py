...
class TestVQE(unittest.TestCase):
    ...
    def test_VQE_SPSA_JW_1qb(self):
        spsa_optimizer = lambda fun, theta0: spsa_minimize(fun, theta0, precision=1e-6,
                                                           maxiter=500, c=0.05, a=1, A=0)
        ...
        qpu = LinAlg()
        energy, _, _, _ = VQE(hamilt_sp, spsa_optimizer, simple_circuit,
                              np.array([random.random() * 2 * np.pi]), qpu)
    ...
    def test_VQE_SPSA_JW_2qb(self):
        ...
        hamilt_sp = transform_to_jw_basis(hamilt)
        qpu = LinAlg()
        calculated_energy, _, _, _ = VQE(hamilt_sp, spsa_optimizer, simple_circuit_twoparameters,
                                         np.array([random.random() * 2 * np.pi, random.random() * 2 * np.pi]), qpu)
