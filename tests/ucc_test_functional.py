...
class TestH2(unittest.TestCase):
    def prepare_h2(self, use_pyscf=False, verbose=False):
        ...
        if use_pyscf:
            ...
            rdm1, orbital_energies, nuclear_repulsion,\
                nels, one_body_integrals, two_body_integrals = perform_pyscf_computation(geometry=geometry,
                                                                                         basis=basis,
                                                                                         spin=0,
                                                                                         charge=0,
                                                                                         verbose=True)
        ...
        return nels, one_body_integrals, two_body_integrals, orbital_energies,\
            nuclear_repulsion, noons

    def test_basic(self, use_pyscf=False, verbose=False):
        nels, one_body_integrals, two_body_integrals,\
            orbital_energies, nuclear_repulsion, noons = self.prepare_h2()
        ...
        H_active_sp = trafo(H_active)
        nqbits = H_active_sp.nbqbits

        # expressing the cluster operator in spin terms
        cluster_ops_sp = [trafo(t_o) for t_o in cluster_ops]
        ...
        res = scipy.optimize.minimize(lambda theta: fun(theta), x0=theta_0,
                                      method='COBYLA', tol=1e-15,
                                      options={'maxiter': 1000})
        self.assertAlmostEqual(res.fun, -1.1372701746609022, delta=1e-8)
        # FCI energy is -1.1372701746609022

    def test_more_basic(self, use_pyscf=False, verbose=False):
        nels, one_body_integrals, two_body_integrals,\
            orbital_energies, nuclear_repulsion, noons = self.prepare_h2(use_pyscf)
        ...
        cluster_ops, theta_0, hf_init = get_cluster_ops_and_init_guess(nels,
                                                                       noons_full,
                                                                       orb_energies_full,
                                                                       H.hpqrs)
