...
from qat.fermion.matchgates import *
...
def product_param_real_part(job, bra_key, ket_key, user_custom_gates=None):
    """
...
def hamiltonian_gradient_param_imag_part(job, hamilt, partial_key, user_custom_gates=None):
    r"""
...
def auto_differentiation_gradient_dictionary(job, hamilt, parameters_dict, user_custom_gates=None):
    """
    ...
    for var_key in parameters_dict:
        gradient_jobs_dict[var_key] = hamiltonian_gradient_param_imag_part(
            job, hamilt, var_key, user_custom_gates=user_custom_gates
        )
