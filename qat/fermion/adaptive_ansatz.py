...
@namespace ...
@authors Nabil Menai <nabil.menai@atos.net>
@authors Satya Bade <satyanarayana.bade@atos.net>
...
class AdaptiveAnsatzPlugin(Junction):
    ...
    def __init__ (self, operator_pool, commutators=None, max_iter=10,
                  verbose=False, use_external_optimizer=False, tol=1e-5,
                  max_vqe_iter=100, method="BFGS"):
    ...
    def evaluate_gradients(self):
        ...
        for op in self.commutators:
            ...
            if self.verbose:
                if abs(val) > 1e-12 : print("Op: %s, <psi|op|psi>= %s"%(op, val))
        return gradients
    ...
    def find_angles(self, job, x0):
        ...
        res = minimize(fun, x0, method=self.method, tol=self.tol,
    ...
    def do_compatibility_check(self, job):
        if job.observable is None:
            raise PluginException(code=ErrorType.ABORT,
        ...
        for i, op in enumerate(self.operator_pool):
            if (op.nbqbits != job.observable.nbqbits):
                raise PluginException(code=ErrorType.ABORT,
                                      ...
                                      but the observable in the job acts on {2} qubit(s)"\
                                      .format(i, op.nbqbits, job.observable.nbqbits))
    ...
    def run(self, job, meta_data):
        ...
        for i in range(self.max_iter):
            ...
            self.circuit = self.parametric_circuit(**{"theta_"+str(j): elm
                                                      for j, elm in enumerate(theta)})
            if self.verbose:
