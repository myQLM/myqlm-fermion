...
def compute_integrals(molecule, mo_coeff, hcore):
    ...
    two_electron_integrals = np.asarray(two_electron_integrals.transpose(0, 2, 3, 1),
                                        order='C')
    return one_electron_integrals, two_electron_integrals


def perform_pyscf_computation(geometry, basis, spin, charge,
                              verbose=False, run_FCI=True):
