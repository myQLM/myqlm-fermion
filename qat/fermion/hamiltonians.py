...
class ElectronicStructureHamiltonian(Hamiltonian):
    ...
    def exponential(self):
        ...
        Returns:
            numpy.ndarray
        """
        return la.expm(-1j * self.get_matrix())
...
def make_embedded_model(
    U, mu, D, lambda_c, t_loc=None, int_kernel=None, grouping="spins"
):
    r"""
    Returns the canonical second quantized form 
    ...
    h_pq = np.zeros((2 * M, 2 * M), dtype="complex")
    ...
    if grouping == "spins":
        ...
        if int_kernel is None and U != 0:
            for i in range(M // 2):
                a = _ind_clusters_ord(2 * i, M)
                b = _ind_clusters_ord(2 * i + 1, M)
                h_pqrs[a, b, a, b] = -U
                h_pqrs[b, a, b, a] = -U
...
def _ind_clusters_ord(ind_spins_ord, M):
    """
...
def _ind_spins_ord(ind_clusters_ord, M):
    """
